...
{
    public:
        TDVPObserver (const Fermion& sites, const MPS& psi, const Args& args = Args::global())
...
inline Real myOnsite_mea (const ITensor& A, const ITensor& op)
{
    ITensor re = A * op;
    re.noPrime ("Site");
    re *= dag(A);
...
void TDVPObserver :: measure (const Args& args)
{
    ...
    if ((nc == 2 && oc == N) || ha == 2)
    {
        ...
        Real ni = myOnsite_mea (psi().A(oc), n_op);
        cout << "\tn " << oc << " = " << ni << endl;
        ...
        if (oc != N)
        {
            ...
            Real j = -2. * imag(eltC (phiJ * dag(phi)));
