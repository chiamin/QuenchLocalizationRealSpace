...
//   --L--
//
tuple<ITensor,ITensor> Orthogonalize (const ITensor& A, const GlobalIndices& IS, Real errGoal=1e-15, int maxIter=10000)
{
    IS.check("A",A);
    auto is = IS.is();
    auto il = IS.il();
...
inline void Rotate_basis (ITensor& C, ITensor& AL, ITensor& AR, const GlobalIndices& IS)
{
    IS.check("C",C);
    IS.check("AL",AL);
    IS.check("AR",AR);
...
inline tuple<ITensor,ITensor,ITensor> MixedCanonical (const ITensor& A, const GlobalIndices& IS, Real errGoal=1e-12, int maxIter=10000)
{
    IS.check("A",A);
    auto il = IS.il();
    auto ir = IS.ir();
    ...
    auto [AL, L]  = Orthogonalize (A, IS, errGoal, maxIter);
    auto ALtmp = swapInds (AL, {il}, {il2});
    auto [AR, C] = Orthogonalize (ALtmp, IS, errGoal, maxIter);
    ...
    Rotate_basis (C, AL, AR, IS);

    assert (iut::check_ortho (AL, il2));
    assert (iut::check_ortho (AR, ir2));
    return {AL, AR, C};
}

//----------------------------------------------------

inline ITensor get_AC (const ITensor& ALR, ITensor C, const GlobalIndices& IS)
{
    IS.check("C",C);
...
inline ITensor get_polarU (const ITensor& T, const IndexSet& iUs, const GlobalIndices& IS)
{
    ITensor U (iUs), S, V;
    svd (T, U, S, V);
...
template <enumLR dir>
inline ITensor get_ALR_2 (const ITensor& AC, const ITensor& C, const GlobalIndices& IS)
{
    ...
    auto UA = get_polarU (AC, {is,i1}, IS);
    auto UC = get_polarU (C, {prime(i1,2)}, IS);
    auto UCdag = dag(UC);
    UCdag.noPrime (prime(i2,2));
    auto ALR = UA * UCdag;

    assert (iut::check_ortho (ALR, prime(i1,2)));
...
inline ITensor get_AL (const ITensor& AC, const ITensor& C, const GlobalIndices& IS)
{
    IS.check("AC",AC);
    IS.check("C",C);
    ...
    auto AL = get_polarU (AC_Cdag, {is,il}, IS);

    IS.check("AL",AL);
    assert (iut::check_ortho (AL, prime(il,2)));
    return AL;
}

inline ITensor get_AR (const ITensor& AC, const ITensor& C, const GlobalIndices& IS)
{
    IS.check("AC",AC);
    IS.check("C",C);
    ...
    auto AR = get_polarU (Cdag_AC, {prime(ir,2)}, IS);

    IS.check("AR",AR);
    assert (iut::check_ortho (AR, prime(ir,2)));
    return AR;
}

template <enumLR dir>
inline ITensor get_LR (const ITensor& C, const GlobalIndices& IS)
{
    IS.check("C",C);
...
template <enumLR dir>
ITensor get_LR_2 (const ITensor& AL, ITensor R, const GlobalIndices& IS, Real crit=1e-15)
{
    if constexpr (dir == RIGHT)
    {
        IS.check("AL",AL);
